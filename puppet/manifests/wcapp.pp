...
class wls1036{
  ...
  wls::installwc{'wcPS7':
    ...
    puppetDownloadMntPoint => $puppetDownloadMntPoint, 
    remoteFile             => hiera('wls_remote_file'),
    wcFile                 => 'ofm_wc_generic_11.1.1.8.0_disk1_1of1.zip',
    require                => Wls::Opatch['17014142_soa_patch'],
  }

  # webcenter content
  wls::installwcc{'wccPS7':
    mdwHome                => $osMdwHome,
    wlHome                 => $osWlHome,
    oracleHome             => $osOracleHome,
    fullJDKName            => $jdkWls11gJDK,  
    ...
    puppetDownloadMntPoint => $puppetDownloadMntPoint, 
    remoteFile             => hiera('wls_remote_file'),
    wccFile1               => 'ofm_wcc_generic_11.1.1.8.0_disk1_1of2.zip',
    wccFile2               => 'ofm_wcc_generic_11.1.1.8.0_disk1_2of2.zip',
    require                => Wls::Installwc['wcPS7'],
  }
...
class wls1036_domain{
  ...
  $reposUrl        = "jdbc:oracle:thin:@wcdb.example.com:1521/test.oracle.com"
  $reposPrefix     = "DEV"
  ...
  wls::wlscontrol{'startAdminServer':
    ...
    require       => Wls::Wlsdomain['wcDomain'],
  }

  # create keystores for automatic WLST login
  wls::storeuserconfig{'wcDomain_keys':
    wlHome        => $osWlHome,
    fullJDKName   => $jdkWls11gJDK,
    domain        => $wlsDomainName, 
    address       => $address,
